# This Makefile is for the opv extension to perl.
#
...
#     ABSTRACT => q[Check content against OpenPGP signatures]
#     AUTHOR => [q[Tom Jones <tom.jones@bccx.com>]]
#     BUILD_REQUIRES => {  }
#     EXE_FILES => [q[bin/opv], q[bin/opv.git]]
#     MAN1PODS => { doc/opv.git.pod=>q[blib/man1/opv.git.1], doc/opv.pod=>q[blib/man1/opv.1] }
#     NAME => q[opv]
#     PREREQ_PM => {  }
#     VERSION => q[0.1]
...
EXE_FILES = bin/opv bin/opv.git

pure_all :: $(INST_SCRIPT)/opv.git $(INST_SCRIPT)/opv
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/opv.git $(INST_SCRIPT)/opv 

$(INST_SCRIPT)/opv.git : bin/opv.git $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/opv.git
	$(CP) bin/opv.git $(INST_SCRIPT)/opv.git
	$(FIXIN) $(INST_SCRIPT)/opv.git
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/opv.git

$(INST_SCRIPT)/opv : bin/opv $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/opv
	$(CP) bin/opv $(INST_SCRIPT)/opv
	$(FIXIN) $(INST_SCRIPT)/opv
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/opv
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               opv' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.1' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Check content against OpenPGP signatures' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Tom Jones <tom.jones@bccx.com>' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Check content against OpenPGP signatures</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Tom Jones &lt;tom.jones@bccx.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
